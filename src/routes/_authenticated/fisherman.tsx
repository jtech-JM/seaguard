...
import { TRIP_STATUS_LABEL, TRIP_STATUS_TONE, type TripStatus } from "@/lib/marine-types";
...
export const Route = createFileRoute("/_authenticated/fisherman")({
  ssr: false,
  beforeLoad: ({ context }) => requireRole(context as any, ["fisherman"]),
  head: () => ({
    meta: [
...
interface Profile {
  ...
  phone: string | null; fisherman_id: string | null; bmu_id: string | null;
}
interface FishermanFull {
  id: string; full_name: string; phone: string | null;
  national_id: string | null; bmu_id: string | null;
  bmu?: { name: string } | null;
}
interface BoatRow { id: string; name: string; registration_number: string | null; boat_type: string | null }
interface DeviceRow { id: string; device_id: string; last_seen_at: string | null; active: boolean }
interface CrewMember { id: string; fisherman_id: string; role: string | null; fisherman?: { full_name: string } | null }
interface Trip {
  id: string; status: TripStatus;
  planned_departure: string | null; actual_departure: string | null;
  expected_return: string | null; actual_return: string | null;
  fishing_area: string | null; destination: string | null; notes: string | null;
  boat?: BoatRow | null;
  crew?: CrewMember[];
}
...
function FishermanPortal() {
  ...
  const [form, setForm] = useState({ destination: "", fishing_area: "", expected_return: "", notes: "" });
  const [detailTrip, setDetailTrip] = useState<Trip | null>(null);

  async function load() {
    const { data: userRes } = await supabase.auth.getUser();
    if (!userRes.user) return;
    const { data: prof } = await supabase.from("profiles").select("*").eq("id", userRes.user.id).maybeSingle();
    setProfile(prof as Profile);
    if (!prof?.fisherman_id) return;
    const [{ data: fm }, { data: bts }, { data: dvs }, { data: trs }] = await Promise.all([
      supabase.from("fishermen").select("*, bmu:bmu_id(name)").eq("id", prof.fisherman_id).maybeSingle(),
      supabase.from("boats").select("*").eq("owner_fisherman_id", prof.fisherman_id).limit(1),
      supabase.from("devices").select("*, boats!inner(owner_fisherman_id)")
        .eq("boats.owner_fisherman_id", prof.fisherman_id).limit(1),
      supabase.from("sea_trips")
        .select("*, boat:boat_id(id,name,registration_number,boat_type), crew:trip_crew(id,fisherman_id,role,fisherman:fisherman_id(full_name))")
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      if (!data.user) return;
      supabase.from("profiles").select("fisherman_id").eq("id", data.user.id).maybeSingle()
        .then(({ data: prof }) => {
          fishermanId = (prof as any)?.fisherman_id ?? null;
          if (!fishermanId) return;
          const ch = supabase.channel("fisherman-trips")
  ...
  const activeTrip = trips.find((t) =>
    t.status === "at_sea" || t.status === "checked_out" || t.status === "pending_approval",
  );
  ...
  async function checkOut() {
    ...
    setBusy(true);
    try {
      await supabase.from("sea_trips").insert({
        ...
        expected_return: form.expected_return ? new Date(form.expected_return).toISOString() : null,
  ...
  async function checkIn(tripId: string) {
    setBusy(true);
    try {
      await supabase.from("sea_trips").update({ status: "returned", actual_return: new Date().toISOString() }).eq("id", tripId);
      await load();
    } finally { setBusy(false); }
  }
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
        {!profile?.fisherman_id ? (
          <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/5 p-6">
            <div className="text-sm font-semibold text-yellow-300">Account not linked to a fisherman record</div>
            <p className="mt-2 text-sm text-foam/70">
              Your BMU officer needs to link your account to your fisherman registration in the BMU console.
            </p>
          </div>
        ) : (
            ...
            <Panel label="Assigned Boat" icon={<Ship className="h-4 w-4 text-tide" />}>
              {boat ? (
                <>
                  ...
                  {boat.registration_number && <div className="mt-1 font-mono text-[11px] text-foam/40">{boat.registration_number}</div>}
                </>
                ...
                <>
                  ...
                  <div className="mt-1 inline-flex items-center gap-1.5 text-[11px] text-foam/60">
                    <span className={`h-1.5 w-1.5 rounded-full ${
                      device.last_seen_at && Date.now() - new Date(device.last_seen_at).getTime() < 900_000
                        ? "bg-tide" : "bg-foam/30"
                    }`} />
            ...
            {activeTrip ? (
              <div className={`rounded-2xl border p-6 ${
                ...
                <div className={`text-[11px] uppercase tracking-wider ${
                  activeTrip.status === "pending_approval" ? "text-yellow-300"
                  : activeTrip.status === "at_sea" ? "text-tide" : "text-foam/60"
                }`}>
                  Current trip · {TRIP_STATUS_LABEL[activeTrip.status]}
                </div>
                <div className="mt-2 text-lg font-semibold">{activeTrip.destination ?? "At sea"}</div>
                <div className="mt-1 text-sm text-foam/70">
                  Departed: {activeTrip.actual_departure ? new Date(activeTrip.actual_departure).toLocaleString() : "Pending"}
                </div>
                <div className="text-sm text-foam/70">
                  Expected return: {activeTrip.expected_return ? new Date(activeTrip.expected_return).toLocaleString() : "—"}
                </div>
                ...
                {activeTrip.status === "pending_approval" ? (
                  ...
                  <button onClick={() => checkIn(activeTrip.id)} disabled={busy}
              ...
              <div className="rounded-2xl border border-foam/10 bg-foam/[0.03] p-6">
                ...
                <p className="mt-1 text-xs text-foam/50">Submitted to your BMU officer for approval before departure.</p>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <Field label="Destination" value={form.destination} onChange={(v) => setForm({ ...form, destination: v })} />
                  <Field label="Fishing area" value={form.fishing_area} onChange={(v) => setForm({ ...form, fishing_area: v })} />
                  <Field label="Expected return" type="datetime-local" value={form.expected_return}
          ...
          <div className="overflow-hidden rounded-2xl border border-foam/10">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-foam/5">
                {trips.length === 0 && (
                  <tr><td colSpan={5} className="px-4 py-6 text-center text-foam/50">No trips yet</td></tr>
                )}
                {trips.map((t) => {
                  const tone = tripTone(t.status);
                  return (
                    <tr key={t.id} className="hover:bg-foam/[0.03] cursor-pointer" onClick={() => setDetailTrip(t)}>
                      ...
                      <td className="px-4 py-2">{t.actual_departure ? new Date(t.actual_departure).toLocaleString() : "—"}</td>
                      <td className="px-4 py-2">{t.actual_return ? new Date(t.actual_return).toLocaleString() : "—"}</td>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-2xl border border-foam/15 bg-ocean p-5 text-foam">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded p-1 text-foam/60 hover:bg-foam/10">✕</button>
        </div>
        <div className="space-y-3 text-sm">
          ...
          <Row label="Planned departure" value={trip.planned_departure ? new Date(trip.planned_departure).toLocaleString() : "—"} />
          <Row label="Actual departure" value={trip.actual_departure ? new Date(trip.actual_departure).toLocaleString() : "—"} />
          <Row label="Expected return" value={trip.expected_return ? new Date(trip.expected_return).toLocaleString() : "—"} />
          <Row label="Actual return" value={trip.actual_return ? new Date(trip.actual_return).toLocaleString() : "—"} />
...
function Panel({ label, icon, children }: { label: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-foam/10 bg-foam/[0.03] p-5">
      <div className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-foam/50">{icon}{label}</div>
